...
.center-mark {
  position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); font-weight: 800;
}
.shake-text {
...
.reversal-mark {
  font-size: 88px; color: #ffd166; text-shadow: 0 0 20px rgba(255, 209, 102, 0.8);
}
.lightning {
  position: absolute; top: 0; width: 4px; height: 100%;
  background: linear-gradient(transparent, #fff, #ffd700, transparent);
  animation: flash 0.6s ease infinite; opacity: 0;
}
.heart {
  position: absolute; bottom: -30px; color: #ff6b81;
  animation: float-up 3s ease-in forwards;
}
.danmaku {
  position: absolute; white-space: nowrap; color: #fff;
  font-size: 18px; font-weight: bold; text-shadow: 1px 1px 3px rgba(0,0,0,0.8);
  animation: scroll-left linear forwards; right: -280px;
}
.suspense-vignette {
...
.suspense-fog {
  position: absolute; bottom: 0; width: 40%; height: 55%;
  background: linear-gradient(transparent, rgba(124, 58, 237, 0.35));
  filter: blur(12px); animation: drift-fog 2.5s ease-in-out infinite alternate;
}
.suspense-mark { font-size: 80px; color: #c4b5fd; text-shadow: 0 0 24px rgba(124, 58, 237, 0.9); animation: pulse-scale 1s ease-in-out infinite; }
.funny-emoji { position: absolute; bottom: -40px; animation: bounce-up 1.8s ease-out forwards; }
.tear { position: absolute; top: -20px; color: #74c0fc; animation: fall-down 2.8s linear forwards; }
.rage-glow {
  position: absolute; inset: 0;
...
.flame { position: absolute; font-size: 32px; animation: flame-rise 1.6s ease-out forwards; }
.shock-flash { position: absolute; inset: 0; background: rgba(255, 255, 255, 0.85); animation: shock-blink 0.35s ease-out 2; }
.shock-mark { font-size: 96px; color: #333; animation: shock-pop 0.4s ease-out; }
.quote-line {
  position: absolute; white-space: nowrap; color: #63e6be; font-size: 18px; font-weight: 700;
  text-shadow: 0 0 8px rgba(32, 201, 151, 0.8); animation: scroll-left linear forwards; right: -280px;
}
