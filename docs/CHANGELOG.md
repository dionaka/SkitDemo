...
## [2026-06-01] 登录页移动端视口缩放修复

- 窄屏下通过 rem 根字号等比缩小整卡（`min(10px, (100vw-2rem)/90)`），避免手机浏览器默认放大到左上角
- 输入框、按钮改用 rem，保持左右 35%/65% 分栏布局不变

---

## [2026-06-01] 登录页布局对齐原站（手机/桌面统一）

- 移除 `scale(0.62)` 缩放与 `@media` 移动端专用布局
- 与原站一致：`90rem` 左右分栏卡片、`margin: 10% auto`、`<br>` 间距结构
- 手机开/关「桌面版网站」使用同一套布局
